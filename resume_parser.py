...
def call_groq_with_retry(messages, response_format, max_retries=6):
    for attempt in range(max_retries):
        try:
            ...
            if "rate_limit" in str(e).lower() or "429" in str(e):
                ...
                print(f"[RateLimit] Waiting {wait_sec}s before retry (attempt {attempt + 1}/{max_retries})...")
                time.sleep(wait_sec)
            else:
                raise
...
response=call_groq_with_retry(messages=messages, response_format=response_format)
...
class Resume(BaseModel):
    ...
    education: list[str | dict] = []
    projects: list[str | dict] = []
    certifications: list[str | dict] = []
